...
CFLAGS+= $(DEFS) -I/usr/local/arm/3.4.1/arm-linux/include -L/usr/local/arm/3.4.1/arm-linux/lib -Wall -W  -g
###################OBJ文件及路径############################################
EXTENSION=c
...
ifeq ($(CC),arm-linux-gcc)
	...
	#OBJS+=$(OBJ_DIR)/mdct_arm.o
	#OBJS+=$(OBJ_DIR)/wma_arm.o
else
	LIBS+= -lasound
endif
...
#$(OBJ_DIR)/mdct_arm.o:$(SRC_DIR)/mdct_arm.s
#	$(CC) $< -o $@ -c  $(CFLAGS)

#$(OBJ_DIR)/wma_arm.o:$(SRC_DIR)/wma_arm.s
#	$(CC) $<  -o $@ -c  $(CFLAGS)
